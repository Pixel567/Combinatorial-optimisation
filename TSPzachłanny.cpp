...
void greedy(int n,float ** macierz) //algorytm zach�anny dla TSP
{
	...
	float min0=0;
	int i=1;
	for(int j=1;j<=n;j++){macierz[0][j]=macierz[1][j];}
	while(min0!=N)
	{
		cout<<i<<" ";
		min0=N;
		...
		macierz[i][min]=0;
		for(int k=1;k<=n;k++) macierz[k][i]=0;
		if(min0==N) {length+=macierz[0][i]; i=1; cout<<i<<endl;} //ostatni przypadek powr�t do pocz�tku
		length+=min0;
		i=min;
